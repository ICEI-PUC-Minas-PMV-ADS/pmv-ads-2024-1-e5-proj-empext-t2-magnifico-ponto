...
namespace MagnificoPonto.Services
{
    ...
    {
        public static async Task<RestResponse> GetApiResponse(List<ProdutoModel> products, string cepToDeliver)
        {
            ...
            {
                size += Convert.ToDouble(item.Tamanho);
                weight += Convert.ToDouble(item.Peso);
            }

            request.AddHeader("Accept", "application/json");
            request.AddHeader("Authorization", $"Bearer {token}");
            request.AddHeader("User-Agent", "Aplicação (email para contato técnico)");
            request.AddJsonBody($"{{\"from\":{{\"postal_code\":\"30120080\"}},\"to\":{{\"postal_code\":\"{cepToDeliver}\"}},\"package\":{{\"height\":5 ,\"width\":{size},\"length\":{size},\"weight\":{weight}}}}}", false);
